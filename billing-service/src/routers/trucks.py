...
from ..models.schemas import TruckCreate, TruckUpdate, TruckResponse, TruckDetails
...
async def register_truck(truck_data: TruckCreate):
    ...
    try:
        ...
        logger.warning(f"Attempt to register truck {truck_data.id} with non-existent provider {truck_data.provider_id}")
...
async def get_truck_details(
    ...
    try:
        ...
        try:
            weight_item = await weight_client.get_item_details(truck_id, from_date, to_date)
            
            if weight_item is None:
                raise HTTPException(status_code=404, detail="Truck not found in weight service")
            
            return TruckDetails(
                id=weight_item.id,
                tara=weight_item.tara,
