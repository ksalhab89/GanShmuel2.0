...
class WeightServiceClient:
    ...
    async def _make_request(self, method: str, endpoint: str, **kwargs) -> Optional[Dict[Any, Any]]:
            ...
            try:
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Weight service returned {response.status_code} for {endpoint}")
        ...
        # All retries failed
        raise WeightServiceError(f"Weight service unavailable after {self.max_retries} attempts")
    
    async def get_item_details(self, item_id: str, from_date: str, to_date: str) -> Optional[WeightItem]:
        """
        ...
        try:
            ...
            logger.error(f"Error parsing weight service response for item {item_id}: {e}")
            raise WeightServiceError(f"Error processing weight service response")
    
    async def get_transactions(
        self, 
        from_date: str, 
        to_date: str, 
        filter_directions: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
